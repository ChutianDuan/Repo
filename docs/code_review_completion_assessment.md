...
| P2 | SSE 网关每个流式请求 detach 一个 OS thread，并用阻塞 libcurl 读取上游，见 `cpp_gateway/src/handlers/StreamChatHandler.cc:93` 到 `cpp_gateway/src/handlers/StreamChatHandler.cc:114`。 | 少量演示没有问题，但高并发 SSE 会让线程数和连接资源不可控。 | 改为受控线程池、连接计数和超时回收；或者使用支持流式回调的异步 HTTP 客户端。 |
